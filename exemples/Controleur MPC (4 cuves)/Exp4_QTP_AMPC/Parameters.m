...
% Case: AMPC (linearization on actual operation point)
x0 = casadi.MX.sym('IC', nx, 1); % Define symbol for state
u0 = casadi.MX.sym('last_input', nu, 1); % Define symbol for input
PHI = f.jacobian(); % Define function return gradient of system
[A, B] = PHI(x0, u0, []); % Define problem: get Jacobien on operation point
% **Define model here**
f0 = f(x0, u0);
f_lact = casadi.Function('ode',{x, u, x0, u0}, {f0+A*(x-x0)+B*(u-u0)},...
{'state', 'input', 'state_0', 'input_0'}, {'dx'});

MPC.model = integrate_model(f_lact, MPC.dt, 'RK4');
...
function phi = integrate_model(f, dt, integrator)
    ...
    IC = SX.sym(x0,size1_in(f, x0), size2_in(f, x0));
    last_input = SX.sym(u0, size1_in(f,u0), size2_in(f,u0));

    switch integrator
      case 'RK4'
        k1 = f(state, controls, IC, last_input);
        k2 = f(state+dt*k1/2.0,controls, IC, last_input);
        k3 = f(state+dt*k2/2.0,controls, IC, last_input);
        k4 = f(state+dt*k3,controls, IC, last_input);
        states_1 = (state+dt*(k1+2*k2+2*k3+k4)/6.0);
        phi = Function('phi', {state,controls,IC,last_input},{states_1});

      case 'euler'
        states_1 = (state+dt*f(state, controls, IC, last_input));
        phi = Function('phi', {state,controls,IC,last_input},{states_1});
    end
end
